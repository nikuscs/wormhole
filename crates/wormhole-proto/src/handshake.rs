...
pub struct Welcome {
    ...
    /// Public domains accepted by the relay.
...
impl<'a> ClientHandshake<'a> {
    ...
    /// Adds a server-issued enrollment credential to the opening frame.
    #[must_use]
    ...
    pub fn step(&mut self, incoming: &ControlFrame) -> Result<HandshakeStep, ProtoError> {
        match (self.state, incoming) {
            ...
            (
                ...
                ControlFrame::Welcome { session, limits, motd, domains },
            ) => {
...
impl<F: Fn(&str, Option<&str>) -> KeyDecision> ServerHandshake<F> {
    ...
    /// Advertises the relay's accepted public domains after authentication.
    #[must_use]
    ...
    pub fn step(&mut self, incoming: &ControlFrame) -> Result<HandshakeStep, ProtoError> {
        match (self.state, incoming) {
            (ServerState::AwaitingHello, ControlFrame::Hello { proto, pubkey, invite, .. }) => {
                Ok(self.accept_hello(*proto, pubkey, invite.as_deref()))
            }
    ...
    fn accept_hello(
        ...
        self.invite = invite.map(str::to_owned);
    ...
    fn verify_auth(&mut self, signature: &str) -> Result<HandshakeStep, ProtoError> {
        ...
        match (self.is_authorized)(&public_key, self.invite.as_deref()) {
            KeyDecision::Authorized => {}
            KeyDecision::Unknown => return Ok(self.deny(DenyReason::UnknownKey)),
            KeyDecision::Revoked => return Ok(self.deny(DenyReason::KeyRevoked)),
            KeyDecision::Limit => return Ok(self.deny(DenyReason::Limit)),
        }
...
const fn frame_name(frame: &ControlFrame) -> &'static str {
    match frame {
        ...
        ControlFrame::Unbound { .. } => "unbound",
        ControlFrame::ForgetReservation { .. } => "forget_reservation",
        ControlFrame::ForgotReservation { .. } => "forgot_reservation",
