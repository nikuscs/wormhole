...
async fn actor_loop(
    ...
    loop {
        ...
        if let Err(error) = result {
            tracing::warn!(?error, "mux actor input failed");
            break;
        }
        if let Err(error) = flush_outbound(state, channels, io, outbound_permits).await {
            tracing::warn!(?error, "mux actor output failed");
            break;
        }
    }
}
...
async fn flush_outbound(
    ...
    while let Some(channel) = state.take_ready_send_fin() {
...
async fn handle_command(
    ...
    match command {
        ...
        Command::Fin { channel } => finish_send(channel, state),
...
fn finish_send(channel: u32, state: &mut MuxState) {
    let _finished = state.finish(channel, Direction::Send);
}
...
async fn write_network_data(
    ...
    let writer = channels.get(&channel).expect("checked channel").writer.clone();
    if writer.send(WriterCommand::Data(message.payload, Some(permit))).await.is_err() {
        reset_channel(channel, state, channels);
        send_control(&io.outbound, &MuxControl::Reset { channel }).await?;
    }
    Ok(())
