...
fn control_frame() -> impl Strategy<Value = ControlFrame> {
    prop_oneof![
        (any::<u16>(), text(), text()).prop_map(|(proto, client, pubkey)| ControlFrame::Hello {
            proto,
            client,
            pubkey
        }),
        ...
        (identifier(), any::<u32>(), any::<u32>(), option::of(text())).prop_map(
            |(session, max_binds, max_streams, motd)| ControlFrame::Welcome {
                session,
