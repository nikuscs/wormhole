...
#[test]
fn send_fin_waits_until_queued_payload_is_flushed() {
    let mut mux = MuxState::default();
    mux.open(1).expect("open");
    mux.acknowledge(1).expect("ack");
    mux.enqueue(1, b"last payload".to_vec()).expect("enqueue");
    mux.finish(1, Direction::Send).expect("finish");

    assert_eq!(mux.take_ready_send_fin(), None);
    assert_eq!(mux.next_message().expect("payload").payload, b"last payload");
    assert_eq!(mux.take_ready_send_fin(), Some(1));
    assert_eq!(mux.take_ready_send_fin(), None);
}
