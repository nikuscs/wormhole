...
fn every_control_frame_has_a_stable_protocol_name() {
    ...
    let frames = vec![
        ...
        ControlFrame::Welcome { session: id, limits, motd: None, domains: Vec::new() },
...
fn client_and_server_complete_happy_path() {
    ...
    let mut server = ServerHandshake::new(
        ...
        move |presented, _invite| {
            if presented == public_key { KeyDecision::Authorized } else { KeyDecision::Unknown }
        },
    )
    .with_domains(vec!["tun.example.com".to_owned(), "dev.example.com".to_owned()]);
    ...
    assert_eq!(client_welcome.domains, ["tun.example.com", "dev.example.com"]);
}

#[test]
fn server_receives_invite_only_with_the_signed_client_hello() {
    ...
    let mut client = ClientHandshake::new(&identity, "relay.example.com", "test-client")
        .with_invite("whi_public_secret".to_owned());
    let mut server =
        ServerHandshake::new("relay.example.com", limits(), None, move |_key, invite| {
            *captured.lock().expect("capture lock") = invite.map(str::to_owned);
            KeyDecision::Authorized
        });

    let challenge = take_reply(server.step(&client.hello()).expect("hello must succeed"));
    assert!(seen.lock().expect("capture lock").is_none());
    let auth = take_reply(client.step(&challenge).expect("challenge must sign"));
    let _welcome = server.step(&auth).expect("authorization must succeed");

    assert_eq!(seen.lock().expect("capture lock").as_deref(), Some("whi_public_secret"));
}
...
fn revoked_and_limited_keys_receive_specific_denials() {
        ...
        [(KeyDecision::Revoked, DenyReason::KeyRevoked), (KeyDecision::Limit, DenyReason::Limit)]
    {
        let mut client = ClientHandshake::new(&identity, "relay.example.com", "test-client");
        let mut server =
            ServerHandshake::new("relay.example.com", limits(), None, move |_, _| decision);
        let challenge = take_reply(server.step(&client.hello()).expect("challenge"));
        let auth = take_reply(client.step(&challenge).expect("signed challenge"));
        let step = server.step(&auth).expect("denial");
...
fn unknown_key_is_denied() {
    ...
    let mut server =
        ServerHandshake::new("relay.example.com", limits(), None, |_, _| KeyDecision::Unknown);

    let challenge = take_reply(server.step(&client.hello()).expect("challenge"));
    let auth = take_reply(client.step(&challenge).expect("signed challenge"));
    let step = server.step(&auth).expect("denial is a valid handshake step");
...
fn auth_before_hello_is_a_protocol_error() {
    let mut server =
        ServerHandshake::new("relay.example.com", limits(), None, |_, _| KeyDecision::Authorized);
...
fn bad_signature_is_denied() {
    ...
    let mut server =
        ServerHandshake::new("relay.example.com", limits(), None, move |presented, _invite| {
            if presented == public_key { KeyDecision::Authorized } else { KeyDecision::Unknown }
        });
