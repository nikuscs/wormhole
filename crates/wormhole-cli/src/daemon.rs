...
impl DaemonServer {
    pub async fn bind(config_path: Option<&PathBuf>) -> Result<Self, DaemonError> {
        ...
        restore(&state).await;
        Ok(Self { listener, state, lock, socket: paths.socket })
    }

    pub async fn run(self) -> Result<(), DaemonError> {
        let Self { listener, state, lock, socket } = self;
        let _lock = lock;
        let _cleanup = SocketCleanup(socket);
        let shutdown = state.shutdown.clone();
        let signal = shutdown_signal(shutdown.clone());
        let serve = axum::serve(listener, router(state.clone())).with_graceful_shutdown(signal);
        serve.await?;
