...
#[utoipa::path(get, path = "/v1/requests", params(("endpoint" = Option<Uuid>, Query), ("limit" = Option<u32>, Query), ("since" = Option<String>, Query)), responses((status = 200, body = [CapturedRequest])))]
...
#[utoipa::path(get, path = "/v1/requests/{id}", params(("id" = Uuid, Path)), responses((status = 200, body = CapturedRequest), (status = 404)))]
...
#[utoipa::path(post, path = "/v1/requests/{id}/replay", params(("id" = Uuid, Path)), responses((status = 200, body = ReplayResponse), (status = 400), (status = 404)))]
pub async fn replay(
    State(state): State<ApiState>,
    Path(id): Path<Uuid>,
