...
use wormhole_core::Target;

pub const LOCAL_API_PORT: u16 = 52_731;

pub const fn is_reserved_target(target: &Target) -> bool {
    match target {
        Target::Port(port) | Target::HostPort(_, port) | Target::Iface { port, .. } => {
            *port == LOCAL_API_PORT
        }
    }
}
