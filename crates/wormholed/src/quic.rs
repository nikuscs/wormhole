...
async fn authenticate_channel<S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin>(
    ...
    let domains = state.registry.domains().to_vec();
    let mut handshake =
        ServerHandshake::new(server_name, limits, None, move |public_key, invite| {
            authorize_key(&callback_state, &callback_result, public_key, invite, max_streams)
        })
...
fn authorize_key(
    ...
    match state.auth.authorize_or_redeem(public_key, invite) {
