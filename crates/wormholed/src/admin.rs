...
fn router(state: AdminState) -> Router {
    ...
    Router::new()
        ...
        .route("/v1/invites", get(crate::admin_invites::list).post(crate::admin_invites::create))
        .route("/v1/invites/{id}", delete(crate::admin_invites::revoke))
...
pub use crate::admin_invites::{CreateInviteRequest, CreatedInviteResponse, InviteResponse};
...
pub(crate) type AdminResponseError = (StatusCode, Json<ErrorResponse>);

pub(crate) fn internal(error: impl std::fmt::Display) -> AdminResponseError {
    (StatusCode::INTERNAL_SERVER_ERROR, Json(ErrorResponse { error: error.to_string() }))
}

pub(crate) fn bad_request(error: impl std::fmt::Display) -> AdminResponseError {
...
#[openapi(
    ...
    paths(
        ...
        crate::admin_invites::list, crate::admin_invites::create, crate::admin_invites::revoke
    ),
    components(schemas(
        StatusResponse, CertificateExpiry, BindResponse, FailedWebhookResponse, KeyResponse,
        AuthorizeKeyRequest, KeyFingerprint, CreateInviteRequest, CreatedInviteResponse,
        InviteResponse, ErrorResponse
    )),
