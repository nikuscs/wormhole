...
use crate::{
    ...
    edge_types::{apply_index_policy, forwarded_node, is_forwarding_header},
...
async fn route_request(
    ...
    let temporary = handle.persist == wormhole_proto::frames::Persistence::Temporary;
    let apply = |response| apply_index_policy(response, temporary);
    if !authorized(&request, &handle).await {
        match link_decision(&request, &handle, sni) {
            ...
            LinkDecision::Redirect { location, cookie } => {
                return Ok(apply(link_redirect(&location, &cookie)));
            }
            LinkDecision::Denied => {
                return Ok(apply(static_response(StatusCode::FORBIDDEN, "Forbidden")));
            }
            LinkDecision::NotConfigured => return Ok(apply(unauthorized_response(&handle))),
        }
    }
    if handle.state() != BindState::Online {
        if handle.buffer_policy.is_some() {
            let response = match buffer_request(request, &handle, state).await {
            ...
            return Ok(apply(response));
        }
        return Ok(apply(offline_response()));
    }
    proxy_request(request, peer, sni, handle).await.map(apply)
}
...
fn request_head(
    ...
    append_header(&mut headers, "x-forwarded-host", sni);
...
fn control_response<B>(request: &Request<B>) -> Response<EdgeBody> {
