//! Binary discovery and isolated process helpers for ignored end-to-end tests.
...
pub fn run_help(binary: &std::path::Path) -> Result<(), String> {
    let output = Command::new(binary).arg("--help").output().map_err(|error| error.to_string())?;
    if output.status.success() && output.stdout.starts_with(b"Usage:") {
        Ok(())
    } else {
        Err("binary help smoke failed".to_owned())
    }
}
