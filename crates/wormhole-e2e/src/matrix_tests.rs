...
fn http_semantics_survive_typed_tunnel() {
    ...
    assert_header_semantics(&relay, &client, base);
    assert_stream_semantics(&relay, &client, base);
...
fn assert_header_semantics(relay: &TestRelay, client: &TestClient, base: &str) {
    let cookies = relay
        .request_with(
            "semantics.wormhole.test",
            &format!("{base}/cookies"),
    ...
    let compressed = relay
        .request_with("semantics.wormhole.test", &format!("{base}/gzip"), &["--compressed"])
        .expect("compressed response");
    assert_eq!(body_from_output(&compressed), b"compressed hello");
    let head = relay
        .request_with("semantics.wormhole.test", &format!("{base}/head"), &["--head"])
        .expect("HEAD response");
    let head_text = String::from_utf8_lossy(&head.stdout).to_ascii_lowercase();
    assert!(head_text.contains("content-length: 7"));
    for status in [204, 205, 304] {
        let response = relay
            .request("semantics.wormhole.test", &format!("{base}/status/{status}"))
            .expect("empty response");
        assert_eq!(
            status_from_output(response.clone()),
            status,
...
fn assert_stream_semantics(relay: &TestRelay, client: &TestClient, base: &str) {
    let events =
        relay.request("semantics.wormhole.test", &format!("{base}/sse")).expect("SSE response");
    assert_eq!(body_from_output(&events), b"data: first\n\ndata: second\n\n");
    let range = relay
        .request_with(
            "semantics.wormhole.test",
            &format!("{base}/range"),
    ...
    assert_eq!(body_from_output(&range), b"2345");
    assert_eq!(status_from_output(range), 206);
    let large =
        relay.request("semantics.wormhole.test", &format!("{base}/large")).expect("large response");
    assert!(
        large.status.success(),
        "large response failed after {} bytes: {}\n{}",
        body_from_output(&large).len(),
        String::from_utf8_lossy(&large.stderr),
        client.daemon_log()
    );
    assert_eq!(body_from_output(&large).len(), 2 * 1024 * 1024);
    let upload = tempfile::NamedTempFile::new().expect("upload file");
    std::fs::write(upload.path(), vec![b'u'; 2 * 1024 * 1024]).expect("upload body");
    let upload_arg = format!("@{}", upload.path().display());
    let uploaded = relay
        .request_with(
            "semantics.wormhole.test",
            &format!("{base}/upload"),
            &["--request", "POST", "--data-binary", &upload_arg],
        )
        .expect("large upload");
    assert_eq!(body_from_output(&uploaded), b"2097152");
    let disconnected = relay
        .request("semantics.wormhole.test", &format!("{base}/disconnect"))
        .expect("disconnected response");
...
fn body_from_output(output: &std::process::Output) -> &[u8] {
    output
        .stdout
        .iter()
        .rposition(|byte| *byte == b'\n')
        .map_or(output.stdout.as_slice(), |index| &output.stdout[..index])
}
