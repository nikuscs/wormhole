...
const QUIC_CONTROL_KEEPALIVE: Duration = Duration::from_secs(20);
const WEBSOCKET_CONTROL_KEEPALIVE: Duration = Duration::from_mins(1);
...
impl RemoteConn {
    pub async fn connect(remote: &Remote, identity: Identity) -> Result<Arc<Self>, DriverError> {
        ...
        let control_keepalive = control_keepalive(connection.is_some());
...
const fn control_keepalive(has_quic_connection: bool) -> Duration {
    if has_quic_connection { QUIC_CONTROL_KEEPALIVE } else { WEBSOCKET_CONTROL_KEEPALIVE }
}
