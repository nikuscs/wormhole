...
use wormhole_proto::{
    ALPN, ClientHandshake, HandshakeStep, Identity, Welcome,
...
pub async fn connect_remote(
    remote: &Remote,
    identity: &Identity,
) -> Result<(quinn::Endpoint, quinn::Connection, ControlChannel<BoxControlIo>, Welcome), DriverError>
{
    let addresses =
        remote.resolve_addrs().await.map_err(|error| DriverError::Transport(error.to_string()))?;
    connect_remote_addresses(remote, identity, addresses, None).await
}

/// Connects and presents a transient enrollment invite during authentication.
pub async fn connect_remote_with_invite(
    remote: &Remote,
    identity: &Identity,
    invite: &str,
) -> Result<(quinn::Endpoint, quinn::Connection, ControlChannel<BoxControlIo>, Welcome), DriverError>
{
    let addresses =
        remote.resolve_addrs().await.map_err(|error| DriverError::Transport(error.to_string()))?;
    connect_remote_addresses(remote, identity, addresses, Some(invite)).await
}

pub type WsConnect =
    (ControlChannel<BoxControlIo>, Welcome, tokio::sync::mpsc::Receiver<tokio::io::DuplexStream>);

pub async fn connect_remote_ws(
    remote: &Remote,
    identity: &Identity,
) -> Result<WsConnect, DriverError> {
    connect_remote_ws_with_invite(remote, identity, None).await
}

pub async fn connect_remote_ws_with_invite(
    remote: &Remote,
    identity: &Identity,
    invite: Option<&str>,
) -> Result<WsConnect, DriverError> {
    ...
    connect_remote_ws_addresses(remote, identity, addresses, invite).await
}
...
) -> Result<(quinn::Endpoint, quinn::Connection, ControlChannel<BoxControlIo>, Welcome), DriverError>
{
    let mut attempts = addresses
        .into_iter()
        .map(|address| connect_remote_address(remote, identity, address, invite))
...
async fn connect_remote_ws_addresses(
    ...
    let mut attempts = addresses
        .into_iter()
        .map(|address| connect_remote_ws_address(remote, identity, address, invite))
        .collect::<FuturesUnordered<_>>();
    let mut last_error = None;
    while let Some(result) = attempts.next().await {
        match result {
...
) -> Result<(quinn::Endpoint, quinn::Connection, ControlChannel<BoxControlIo>, Welcome), DriverError>
{
    ...
    let connection = endpoint
        ...
        .map_err(|error| DriverError::Transport(error.to_string()))?;
    let (channel, welcome) =
        authenticate(&connection, identity, &remote.server_name, invite).await?;
    Ok((endpoint, connection, channel, welcome))
}
...
async fn connect_remote_ws_address(
    ...
    let (channel, welcome) = authenticate_io(io, identity, &remote.server_name, invite).await?;
    Ok((channel, welcome, endpoint.incoming))
}
...
async fn authenticate(
    ...
    server_name: &str,
    invite: Option<&str>,
) -> Result<(ControlChannel<BoxControlIo>, Welcome), DriverError> {
    let (send, recv) =
    ...
    authenticate_io(io, identity, server_name, invite).await
}
...
async fn authenticate_io(
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        let mut handshake = invite.map_or_else(
            || ClientHandshake::new(identity, server_name, "wormhole-core"),
            |invite| {
                ClientHandshake::new(identity, server_name, "wormhole-core")
                    .with_invite(invite.to_owned())
            },
        );
        channel
        ...
        match handshake.step(&welcome).map_err(|error| DriverError::Protocol(error.to_string()))? {
            HandshakeStep::Done { welcome, .. } => Ok((channel, welcome)),
            HandshakeStep::Failed { reason, .. } => {
