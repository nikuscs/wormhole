...
use crate::{
    ...
    drivers::{
        ...
        cloudflare_named::{
            ...
            find_json_string, find_uuid, named_config, record_route, route_is_owned,
        },
...
impl TunnelDriver for CloudflareDriver {
    ...
    async fn run(
        ...
        if spec.qualifier.as_deref().unwrap_or("quick") == "named" {
            self.run_named(&spec, target, events, stop).await
        } else {
...
fn command_error(action: &str, output: &CommandOutput) -> DriverError {
...
fn discover_cloudflared() -> Option<PathBuf> {
...
fn strings3(first: &str, second: &str, third: &str) -> Vec<String> {
