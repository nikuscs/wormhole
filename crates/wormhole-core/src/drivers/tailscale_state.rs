    ...
    if let Some(binding) = binding_snapshot(&status.stdout, spec, target) {
        if !binding.contains(&target_text) {
            return Err(DriverError::Capability(format!(
                "tailscale public binding conflicts with requested target; existing={binding}; requested={target_text}; set an explicit public_port override"
            )));
        }
        if !owned {
            return Err(DriverError::Capability(format!(
                "tailscale binding already exists but is not owned by wormhole: {binding}; set an explicit public_port override"
            )));
        }
    }
    Ok(())
