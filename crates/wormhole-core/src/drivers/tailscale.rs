...
use crate::{
    ...
    drivers::{
        ...
        tailscale_state::{
            ActiveBindings, cleanup_if_unchanged, forget_ownership, owns_binding, reject_conflict,
            verify_install,
        },
    },
    error::DriverError,
...
pub struct TailscaleDriver {
    ...
    binary: Option<PathBuf>,
    pub(super) ownership_dir: Option<PathBuf>,
    pub(super) https_port_range: HttpsPortRange,
    pub(super) active: ActiveBindings,
}
...
impl TailscaleDriver {
    pub fn system(https_port_range: HttpsPortRange) -> Self {
    ...
    pub fn with_api(api: Arc<dyn TailscaleApi>) -> Self {
        Self::with_api_and_range(api, HttpsPortRange::default())
    }

    #[cfg(test)]
    pub fn with_api_and_range(
        api: Arc<dyn TailscaleApi>,
        https_port_range: HttpsPortRange,
    ) -> Self {
        Self::with_api_ownership_and_range(api, None, https_port_range)
    }
    ...
    pub fn with_api_and_ownership(
        ...
        Self::with_api_ownership_and_range(api, ownership_dir, HttpsPortRange::default())
    }

    #[cfg(test)]
    fn with_api_ownership_and_range(
        api: Arc<dyn TailscaleApi>,
        ownership_dir: Option<PathBuf>,
        https_port_range: HttpsPortRange,
    ) -> Self {
    ...
    pub(super) async fn reject_conflict(
        &self,
        mode: &str,
        spec: &EndpointSpec,
        target: ResolvedTarget,
...
impl TunnelDriver for TailscaleDriver {
    ...
    async fn run_controlled(
        ...
        let _claim = self.select_binding(&mut spec, target).await?;
