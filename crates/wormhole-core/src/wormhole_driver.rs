...
use crate::{
    ...
    wormhole_connect::connect_transport,
};
...
/// Result of authenticating to a configured relay.
pub struct RemoteInspection {
    pub latency: Duration,
    pub domains: Vec<String>,
}

/// Performs a full authenticated remote handshake and returns relay metadata.
pub async fn inspect_remote(
    remote: &Remote,
    identity: wormhole_proto::Identity,
) -> Result<RemoteInspection, DriverError> {
    let started = std::time::Instant::now();
    let connected = connect_transport(remote, &identity).await?;
    let latency = started.elapsed();
    if let Some(connection) = connected.connection {
        connection.close(0_u32.into(), b"remote inspection complete");
    }
    if let Some(endpoint) = connected.endpoint {
        endpoint.wait_idle().await;
    }
    Ok(RemoteInspection { latency, domains: connected.domains })
}
...
pub async fn test_remote(
    ...
    Ok(inspect_remote(remote, identity).await?.latency)
}
